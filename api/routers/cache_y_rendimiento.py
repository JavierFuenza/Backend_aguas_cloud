...
async def warm_up_cache():
    ...
    try:
        ...
        queries = [
            (
                "SELECT SUM(row_count) as total FROM sys.dm_db_partition_stats WHERE object_id = OBJECT_ID('dw.Mediciones_full') AND index_id IN (0,1)",
                None,
            ),
            ("SELECT COUNT(*) as total_puntos_unicos FROM dw.Puntos_Mapa", None),
            (
                "SELECT DISTINCT Region FROM dw.Puntos_Mapa WHERE Region IS NOT NULL ORDER BY Region",
                None,
            ),
        ]
