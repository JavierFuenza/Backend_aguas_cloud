...
def log_result(endpoint, method, status, time_ms, error=None):
    ...
    print(
        f"{status_icon} {Fore.CYAN}[{method}]{Style.RESET_ALL} {endpoint.ljust(60)} "
...
def save_reports(df):
    ...
    with open(txt_filename, "w", encoding="utf-8") as f:
        f.write(
            f"REPORTE DE EJECUCIÓN API - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
        )
        f.write("=" * 60 + "\n\n")
        ...
        for _, row in endpoint_stats.iterrows():
            f.write(
                f"{row['Method']} {row['Endpoint'][:40].ljust(40)} | Prom: {row['Avg_Time']:.2f}ms | Éxito: {row['Pass_Count']}/{row['Count']}\n"
            )
        ...
        for _, row in slowest.iterrows():
            f.write(
                f"{row['Time_ms']:.2f} ms | {row['Method']} {row['Endpoint']} (Run {row.get('Run_ID', 1)})\n"
            )
...
def run_tests():
    print(
        f"{Fore.YELLOW}=== RUN {current_run_id} | Iniciando Pruebas: Aguas Transparentes API ==={Style.RESET_ALL}"
    )
    ...
    if cuencas_data and "cuencas" in cuencas_data and len(cuencas_data["cuencas"]) > 0:
        candidates = [
            c for c in cuencas_data["cuencas"] if c["cod_subsubcuenca"] is not None
        ]
        if candidates:
            ...
            print(
                f"    -> Usando Cuenca ID: {cuenca_sample}, Sub: {subcuenca_sample}, SubSub: {subsubcuenca_sample}"
            )
        else:
    ...
    if puntos_data and isinstance(puntos_data, list) and len(puntos_data) > 0:
        ...
        print(
            f"    -> Usando Punto UTM: N:{punto_sample['utm_norte']} E:{punto_sample['utm_este']}"
        )
    ...
    if cuenca_sample:
        check_endpoint(
            "/cuencas/stats",
            params={"cod_cuenca": cuenca_sample, "include_global": True},
        )
        check_endpoint(
            "/cuencas/cuenca/series_de_tiempo/caudal",
            params={"cuenca_identificador": cuenca_sample},
        )
        check_endpoint(
    ...
    if subcuenca_sample:
        check_endpoint(
            ...
            params={"cuenca_identificador": subcuenca_sample},
        )
        check_endpoint(
    ...
    if subsubcuenca_sample:
        check_endpoint(
            ...
            params={"cuenca_identificador": subsubcuenca_sample},
        )
        check_endpoint(
    ...
    check_endpoint(
        "/cuencas/shac/series_de_tiempo/caudal", params={"shac_identificador": "499"}
    )
    ...
    check_endpoint(
        "/puntos/count", params={"apr": True, "id_junta": 1.0, "shac": 121}
    )  # Filtros nuevos
    check_endpoint(
        "/puntos/count", params={"id_tipo_extraccion": 2}
    )  # Extracción superficial/subterránea
    check_endpoint(
        "/puntos/count", params={"search": "obracod123"}
    )  # Búsqueda por obra
    ...
    if punto_sample:
        ...
        check_endpoint(
            "/puntos/series_de_tiempo/caudal",
            params={"utm_norte": norte, "utm_este": este},
        )
        check_endpoint(
            "/puntos/series_de_tiempo/altura_linimetrica",
            params={"utm_norte": norte, "utm_este": este},
        )
        check_endpoint(
            "/puntos/series_de_tiempo/nivel_freatico",
            params={"utm_norte": norte, "utm_este": este},
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Pruebas de Carga Funcional para Aguas Transparentes API"
    )
    parser.add_argument(
        "--runs", type=int, default=None, help="Número de veces a ejecutar las pruebas"
    )
    ...
    if args.target:
        ...
        env_choice = (
            input("¿Deseas probar el entorno Local (L) o Producción (P)? [L/P]: ")
            .strip()
            .upper()
        )
        if env_choice == "P":
            is_local = False
            prod_input = input(
                "Ingresa la URL de producción (Ej: https://tu-api.azurewebsites.net): "
            ).strip()
            BASE_URL = (
                prod_input
                if prod_input
                else "https://aguastransparentes.azurewebsites.net"
            )
        else:
            is_local = True
    ...
    if runs is None:
        user_input = input(
            "¿Cuántas veces deseas repetir las pruebas? (Enter para 1): "
        )
        try:
    ...
    if is_local:
        ...
        for _ in range(30):
            ...
            if server_process.poll() is not None:
                print(
                    f"{Fore.RED}Error: Uvicorn se cerró inesperadamente. ¿Quizás el puerto 8000 ya está en uso?{Style.RESET_ALL}"
                )
                sys.exit(1)
        ...
        if not server_ready:
            print(
                f"{Fore.RED}Error: El servidor local no se inició a tiempo.{Style.RESET_ALL}"
            )
            if server_process:
    ...
    print(
        f"{Fore.YELLOW}=== Iniciando Pruebas de Carga Funcional: Aguas Transparentes API ==={Style.RESET_ALL}"
    )
    ...
    try:
        ...
        print(
            f"{Fore.RED}ERROR CRÍTICO: No se pudo conectar a {BASE_URL}. Asegúrate de que la API esté corriendo.{Style.RESET_ALL}"
        )
